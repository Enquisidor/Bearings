...
Append findings to `.logs/issues.md` using the format in `logs/issue-log-format.md`. Every finding must include: severity, category, title, description of the vulnerability, exact file path and line, an exploitation scenario (how an attacker exploits this concretely), and a specific actionable remediation.
...
**FAIL** on any P0 or P1 finding. **PASS-WITH-FINDINGS** on P2/P3 only. **PASS** on no findings.
