...
Append findings to `.logs/issues.md` using the format in `logs/issue-log-format.md`. Every finding must cite the specific WCAG success criterion violated (e.g., "WCAG 2.2 SC 1.3.1 Info and Relationships"). Every finding must include: severity, WCAG reference, description of the violation, exact file path and line, and a specific remediation.
