...
**Decision log** (`logs/decision-log-format.md`): Write an entry for every non-trivial design decision — aggregate boundary placements, consistency model selections, schema denormalization choices, API versioning decisions, integration pattern selections. If you made more than five decisions on a task, that is normal; log all of them.

**Activity log** (`logs/activity-log-format.md`): Write one entry per task summarizing what was produced, what decisions were made, and what remains open.
...
## Bearings — Hard Architectural Constraints

These are non-negotiable. Any spec, contract, or schema you produce that implies the following must be rejected and escalated to the PM before proceeding.

- **No traditional auth system.** No passwords, OAuth, or full account systems. A lightweight magic link or passphrase mechanism for cloud sync access (Step 4) is acceptable — but no email-based account creation, no session tokens stored server-side, no account recovery flows. Pseudonymous IDs only.
- **No database.** Postgres, Supabase, Firestore, MySQL, Redis, SQLite (server-side), and all equivalents are permanently out of scope. The only server-side storage is Cloudflare R2 object blobs, holding encrypted data the server cannot decrypt.
- **No analytics, telemetry, or tracking.** No PostHog, Mixpanel, Google Analytics, Amplitude, Sentry user tracking, or equivalent. Error logging that captures user content is also out of scope.
- **No sharing, social, or export-to-social features.**
- **One thin server role:** the server proxies the Anthropic API and (in Step 4) stores encrypted blobs. Nothing else.

If a proposed feature requires any of the above to implement correctly, flag it as out of scope rather than designing around it.
