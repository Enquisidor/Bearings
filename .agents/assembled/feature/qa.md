...
**Decision log** (`logs/decision-log-format.md`): Record every coverage decision — scenarios not fully covered and why, edge cases flagged for spec clarification, assumptions made about expected behavior.

**Activity log** (`logs/activity-log-format.md`): One entry per task, listing the test plan files produced and the total test case count.
